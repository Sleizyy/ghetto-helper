...
local window_page = 1
local currentPage = (fa["HOUSE"] .. u8 ' Ãëàâíîå ìåíþ')
        ...
        downloadUrlToFile(update_url, update_path, function(id, status)
            if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                ...
                if tonumber(updateIni.info.version) > scriptVersion then
                    msg(tag .. 'Äîñòóïíî îáíîâëåíèå! Âåðñèÿ: ' .. updateIni.info.version_text,
                        -1)
                    update_state = true
                end
    ...
    while true do
        ...
        if update_state then
            downloadUrlToFile(script_url, script_path, function(id, status)
                if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                    msg(tag..'Ñêðèïò óñïåøíî îáíîâëåí!', 1)
                    thisScript():reload()
                end
            end)
            break
...
local newFrame = imgui.OnFrame(function() return render_window[0] end,
    function(player)
        ...
        if addons.AnimButton(fa["HOUSE"] .. u8 ' Ãëàâíîå ìåíþ##1', imgui.ImVec2(120, 30)) then
            window_page = 1
            currentPage = (fa["HOUSE"] .. u8 ' Ãëàâíîå ìåíþ')
        end
        if addons.AnimButton(fa["GEARS"] .. u8 ' Íàñòðîéêè##2', imgui.ImVec2(120, 30)) then
            window_page = 2
            currentPage = (fa["GEARS"] .. u8 ' Íàñòðîéêè')
        end
        if addons.AnimButton(fa["INFO"] .. u8 ' Èíôîðìàöèÿ##3', imgui.ImVec2(120, 30)) then
            window_page = 3
            currentPage = (fa["INFO"] .. u8 ' Èíôîðìàöèÿ')
        end
        if addons.AnimButton(fa["LIST"] .. u8 ' Ôóíêöèè##4', imgui.ImVec2(120, 30)) then
            window_page = 4
            currentPage = (fa["LIST"] .. u8 ' Ôóíêöèè')
        end
        ...
        if window_page == 1 then
            ...
            imgui.LinkText('https://vk.com/ses1404', fa["FILE"] .. u8 ' Ññûëêà íà áëàñòõàê')
            imgui.LinkText('https://vk.com/ses1404', fa["MESSAGE"] .. u8 ' Ñâÿçü ñ ðàçðàáîò÷èêîì')
            imgui.LinkText('https://vk.com/ses1404', fa["FILE_CODE"] .. u8 ' Ôàéë íà ÃèòÕàáå')
