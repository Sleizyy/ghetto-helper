...
local window_page = 1
local currentPage = (fa["INFO"] .. u8 ' Èíôîðìàöèÿ')
        ...
        downloadUrlToFile(update_url, update_path, function(id, status)
            if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                ...
                if tonumber(updateIni.info.version) > scriptVersion then
                    msg(tag .. 'Äîñòïóíî îáíîâëåíèå! Âåðñèÿ: ' .. updateIni.info.version_text,
                        -1)
                    update_state = true
                end
            end
    ...
    while true do
        ...
        if update_state then
            downloadUrlToFile(script_url, script_path, function(id, status)
                if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                    msg(tag..'Ñêðèïò óñïåøíî îáíîâë¸í­!', 1)
                    thisScript():reload()
                end
            end)
            break
...
local newFrame = imgui.OnFrame(function() return render_window[0] end,
    function(player)
        ...
        if addons.AnimButton(fa["INFO"] .. u8 ' Èíôîðìàöèÿ##1', imgui.ImVec2(120, 30)) then
            window_page = 1
        ...
        if addons.AnimButton(fa["INFO"] .. u8 ' Äëÿ 9-10 ðàíãîâ##3', imgui.ImVec2(120, 30)) then
            window_page = 3
            currentPage = (fa.user .. u8 ' Äëÿ 9-10 ðàíãîâ')
        end
        ...
        if window_page == 1 then
            ...
            imgui.LinkText('https://vk.com/ses1404', fa["FILE"] .. u8 ' Òåìà ñ áëàñòõàêà')
            imgui.LinkText('https://vk.com/ses1404', fa["MESSAGE"] .. u8 ' Ìîé âê')
            imgui.LinkText('https://vk.com/ses1404', fa["FILE_CODE"] .. u8 ' Òåìà ñ ãèòõàáà')
